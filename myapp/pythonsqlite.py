...
def clean_field(field):
    ...
    # Check if the field is in date format 'MM/DD/YY'
    if '/' in field:
        try:
            # Parse the date and convert it to 'YYYY-MM-DD' format
            date_obj = datetime.strptime(field, '%m/%d/%y')
            field = date_obj.strftime('%Y-%m-%d')
        except ValueError:
            # Handle invalid date format gracefully, you can set it to None or any default value
            field = None  # Or raise an exception, depending on your use case
    return field
...
with open('gl.csv', 'r') as csv_file:
    ...
    for row in csv_reader:
        ...
        if len(cleaned_row) == 19:  # Skip incomplete rows
            ...
            query = 'INSERT OR REPLACE INTO data VALUES ({})'.format(
                num_placeholders)
